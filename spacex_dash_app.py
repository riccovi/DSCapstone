...
def get_pie_chart(entered_site):
    filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site][['Launch Site', 'class']]
    if entered_site == 'ALL':
...
def get_scatter_plot(entered_site,payload_value):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Scatter Plot for all Sites')
        return fig
    else:
        fig = px.scatter(site_filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title=f'Scatter Plot for {entered_site}')
        return fig
